...
class BuildingCommandController extends CommandController
{
    ...
    private function doCommandSaveoverview()
    {
        ...
        $width = BuildingImageResource::BUILDING_OVERVIEW_WIDTH_DEFAULT;
        $height = BuildingImageResource::BUILDING_OVERVIEW_HEIGHT_DEFAULT;
        if ( $this->getQueue() )
        {
        ...
        $buildingImagePath = Resource::image()->building()->getBuildingOverview(
        ...
        if ( $this->getQueue() )
        {
            $this->getDaoContainer()->getQueueDao()->remove( $this->getQueue()->getId() );
        }
    ...
    public function before()
    {
        ...
        if ( $this->isCommandSaveoverview() )
        {
            ...
            // Set Queue
            $this->setQueue( $this->getDaoContainer()->getQueueDao()->get( self::getQueueId() ) );
        }
    ...
    public function request()
    {
        ...
        DebugHandler::doDebug( DebugHandler::LEVEL_LOW,
                new DebugException( "BuildingCommandController request", self::getCommand(),
