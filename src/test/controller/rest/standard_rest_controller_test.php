...
abstract class StandardRestControllerTest extends ControllerTest
{
    ...
    {
        return array ( StandardRestController::$POST_OBJECT => get_object_vars( $model ) );
    }
...
//                 $this->showSource();
        ...
        if ( $this->assertResponse( AbstractController::STATUS_OK, "Should be correct response" ) )
        {
            ...
            $modelRest = $this->getRestModel(
                    Core::arrayAt( $dataArray, StandardRestView::$FIELD_SINGLE, array () ) );

            // Get REST List
            $modelListRest = $this->getRestModelList(
                    Core::arrayAt( $dataArray, StandardRestView::$FIELD_LIST, array () ) );
...
//                                 $this->showSource();
    ...
    public function testShouldEditSingle()
    {
        ...
        // Assert response
        if ( $this->assertResponse( AbstractController::STATUS_CREATED, "Should be correct response" ) )
        {
    ...
    public function testShouldGetListMultiple()
    {
        ...
        $url = self::getRestWebsite(
                $this->getQueryGetSingle(
                        sprintf( "%s%s%s", $modelOneId, StandardRestController::$ID_SPLITTER, $modelTwoId ) ) );
