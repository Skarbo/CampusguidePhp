...
class ViewFacilityFacilitiesCmsPageMainView extends PageCmsPageMainView
{
    ...
    {
        return self::$ID_VIEW_FACILITY_PAGE_WRAPPER;
    }
    ...
    //     /**
    //      * @see FacilityFacilitiesCmsPageMainView::draw()
    //      */
    ...
    //                 Xhtml::a( "Delete" )->href(
    //                         Resource::url()->cms()->facility()->getDeleteFacilityPage(
    ...
    //         $buttonsGui->addContent(
    //                 Xhtml::a( "Edit" )->href(
    //                         Resource::url()->cms()->facility()->getEditFacilityPage( $this->getFacility()->getId(),
    //                                 $this->getView()->getController()->getMode() ) )->title( "Edit" )->addClass(
    ...
    //                 Xhtml::div( $headerTable )->class_( Resource::css()->cms()->page()->getHeaderWrapper() ) );
    ...
    protected function drawBody( AbstractXhtml $root )
    {
        ...
        $this->drawBuildings( $buildingsMapWrapper );
        $this->drawMap( $buildingsMapWrapper );
        $root->addContent( $buildingsMapWrapper );
    ...
    private function drawBuildings( AbstractXhtml $root )
    {
                ...
                Xhtml::h( 3,
                        sprintf( "%s (%s)",
                                $this->getLocale()->quantity( $this->getFacility()->getBuildings() == 1,
                                        $this->getLocale()->building()->building,
                                        $this->getLocale()->building()->buildings ),
        ...
        for ( $this->getBuildings()->rewind(); $this->getBuildings()->valid(); $this->getBuildings()->next() )
        {
            ...
            // Create row
            $row = Xhtml::tr()->id( sprintf( "building_%s", $building->getId() ) )->class_(
                    "buildings_row_building" );
            ...
            // ... Image
            $img = Xhtml::img()->src(
                    Resource::url()->cms()->buildings()->getBuildingOverviewImage( $building->getId(), 100, 75,
                    ...
                    sprintf( "%s %s", $building->getFloors(),
                            strtolower(
                                    $this->getLocale()->quantity( $building->getFloors(),
                                            $this->getLocale()->building()->floor,
                                            $this->getLocale()->building()->floors ) ) ) )->class_( "floors" );
        ...
        $foot = Xhtml::tfoot( Xhtml::tr( Xhtml::td( "No buildings" )->colspan( 2 ) ) )->class_(
                $this->getFacility()->getBuildings() > 0 ? Resource::css()->cms()->getHide() : "buildings_table_footer" );
